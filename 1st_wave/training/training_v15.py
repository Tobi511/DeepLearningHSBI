...
#---------------------------------------------------------------------------------------------------------
# # v14, aber in den letzten 3 schichten auch schon l2 regularisierung
#---------------------------------------------------------------------------------------------------------
...
model = keras.Sequential([
    ...
    layers.Conv2D(64, 3, activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.0001)),
    layers.MaxPooling2D(pool_size=2),

    layers.Conv2D(128, 3, activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.0001)),
    layers.MaxPooling2D(pool_size=2),

    layers.Conv2D(256, 3, activation='relu', padding='same', kernel_regularizer=regularizers.l2(0.0001)),
    layers.MaxPooling2D(pool_size=2),
...
model.save("models\\model_v15.h5")
...
plt.tight_layout()
plt.savefig("results\\results_v15\\trainingsverlauf_v15.png")  # <- Speichert den Plot als Bild

# Ergebnisse in eine Textdatei speichern
results_dir = "../results/results_v15"
os.makedirs(results_dir, exist_ok=True)

with open(os.path.join(results_dir, "trainingsverlauf_v15.txt"), "w") as f:
